...
describe('PastebinComponent', () => {
  ...
  beforeEach(() => {
    ...
    spy = spyOn(pastebinService, 'getPastebin').and.returnValue(
  ...
  it('should not show the pastebin before OnInit', () => {
    ...
    expect(this.tbody.innerText.replace(/\s\s+/g, '')).toBe(
      '',
  ...
  it('should show the list after getPastebin promise resolves', async () => {
    ...
    fixture.whenStable().then(() => {
      ...
      expect(element.innerText.replace(/\s\s+/g, ' ')).toContain(
        mockPaste[0].title
      );
    });
  });
